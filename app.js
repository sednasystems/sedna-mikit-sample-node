...
const helpers = require('./sedna_helpers.js')
const thirdPartyApi = require('./third_party.js')
const app = express()
const port = 3000
...
const actionToHandler = {
  ...
  'submit': (req, res) => {
    ...
    var form = helpers.getFormValues(req.body.blocks)
    if (form.todoName.length > 50) {
      ...
      blocks = helpers.updateBlockById(req.body.blocks, 'todoName', {error: 'Must be less than 50 characters'})
      const response = helpers.buildResponse(blocks)
      console.log('Responding with validation error')
      res.json(helpers.buildResponse(blocks))
    } else {
      thirdPartyApi.createTodo(form.todoName, form.todoDescription, form.todoDate, () => {
